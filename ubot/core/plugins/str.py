...
async def absen(client, message):
    await message.reply("<b>siapp boss zull🔥 >,< 🔥</b>")
...
async def akugtgkn(client, message):
    await message.reply("<b>Iyaaa zull gantengg bangettt😘😍🥰</b>")


async def rain(client, message):
    await message.reply("<b>Punyaa zull😘</b>")
        ...
        if "secretMsg" in txt:
            ...
            if message.from_user.id not in user_or_me:
                return await send.edit(
                    f"<b>❌ Apasihh apaa?🖕 <a href=tg://user?id={message.from_user.id}>{message.from_user.first_name} {message.from_user.last_name or ''}</a>"
                )
            else:
