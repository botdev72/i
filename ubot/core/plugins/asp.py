...
async def photo_cewek(client, message):
    ...
    try:
        ...
        await photo.copy(
            message.chat.id,
            caption=f"<b>🥰Cewe Nya <a href=tg://user?id={client.me.id}>{client.me.first_name} {client.me.last_name or ''}</a></b>",
            reply_to_message_id=message.id,
        )
        await y.delete()
    except Exception as error:
...
async def photo_cowok(client, message):
    ...
    try:
        ...
        await photo.copy(
            message.chat.id,
            caption=f"<b>🥰Cowo Nya <a href=tg://user?id={client.me.id}>{client.me.first_name} {client.me.last_name or ''}</a></b>",
            reply_to_message_id=message.id,
        )
        await y.delete()
    except Exception as error:
...
async def qris(client, message):
    y = await message.reply_text("<code>bentar</code>")
    anime_channel = random.choice(["@qrispunyazull", "@qrispunyazull"])
    try:
        animenya = []
        async for anime in client.search_messages(
            anime_channel, filter=MessagesFilter.PHOTO
